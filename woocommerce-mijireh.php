...
if(!defined('MJ_WOO_PLUGIN_PATH')) {
  ...
  define('MJ_WOO_PLUGIN_PATH', WP_PLUGIN_DIR . '/' . basename(dirname($mj_plugin_file)) . '/');
  define('MJ_WOO_PLUGIN_URL', WP_PLUGIN_URL . '/' . basename(dirname($mj_plugin_file)) . '/');
}
...
function woocommerce_mijireh_checkout_init() {
	...
	class WC_Mijireh_Checkout extends WC_Payment_Gateway {
      ...
      if($slurp_page_installed != 1) {
        ...
        add_option('slurp_page_installed', 1);
      }
    }
    ...
    public function process_payment($order_id) {
      ...
      foreach($items as $item) {
        ...
        $mj_order->add_item($item['name'], $wc_order->get_item_subtotal($item), $item['qty'], $product->sku);
      }
